...
def SimulateRunsOnCPSR(args):
    ...
    for i in range(int(runs)):
        ...
        while count < Parameter.LengthOfAction:
            ...
            dataCollector.AddData(aid=aid, oid=oid, r=rid, ActOb=ActOb)
            count = count + 1
...
def SimulateTrainDataMultiProcess(args):
    ...
    for i in range(runs):
        ...
        count = 0
        while not Env.isTerminate() and count < maxRunLength:
            if isRandom:
...
def ParseAoSequenceTHMats(aoSequence, THMat, HistMat):
    ...
    for i in range(size):
        ...
        if len(hist) > Parameter.maxHistLen:
            hist.remove(hist[0])
        # back to java program
        test = aoSequence[i + 1::]
        if len(test) > Parameter.maxTestlen or len(test) == 0:
            continue
...
def setNullHistoryAOMats(aoSequence, aoMats):
    ...
    if len(test) > Parameter.maxTestlen:
        return aoMats

...
def loadGroundTruePrediction(file, numberOfActions, numberOfObservations, gameName, rewardDict):
    ...
    with open(file=dir + '/groundTruePredictions/' + fgameName + "/" + file) as f:
        for line in f:
            strs = line.split(" ")
            action = strs[0]
...
def WriteFile(test, Preds, GameName, epoch, numActions, numObservations, predictive_state, rewardDict):
    ...
    # loss = []
    f = open(file=dir + "/observations/Epoch " + str(epoch) + "/" + test + '.txt', mode="w")
            ...
            for ao in Preds.keys():
                ...
                if GameName == "StandTiger":
                    if Parameter.introduceReward:
                        ...
                        Exception("StandTiger needs reward signals")
                else:
                    Exception("Game not seen")
                ...
                for ActOb in Episode:
                    ...
                    if Env.getGameName() == "Tiger95":
                        if r == 10:
                            ...
                            if r != -1:
                                Exception("Reward " + str(r) + " not seen")
...
# game: 'standtiger' or 'tiger95' or 'maze'
# epochs: int
def train(game, epochs):
    ...
    if game == "standtiger":
        game = StandTiger()
        file = "PSR/train/setting/StandTiger.json"
    if game == "tiger95":
        game = Tiger95()
        file = "PSR/train/setting/Tiger95.json"
    if game == "maze":
        game = Maze()
        file = "PSR/train/setting/Maze.json"
    print("Game environment is: " + game.getGameName())
    ...
    while iterNo < epochs:
        ...
        modelQualityOnStandTiger(psrModel=psrModel, epoch=iterNo, StandTiger=game,
...
if __name__ == "__main__":
    train(game='standtiger', epochs=10)